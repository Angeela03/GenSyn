...
for ind, row in read_top_50.iterrows():
    ...
    final_county_population_cp, data_cp, group_by_copula, tae_cp, cp_intermediate = prior_calculations(prior_copula, "entropy_conditional", tau)

    final_county_population_combined, data_combined, group_by_combined, tae_combined, combined_intermediate = prior_calculations(prior_all, "entropy_conditional", tau)
    ...
    with open(os.path.join(data_path, "tae_scores_copula_"+"_.csv"), "a") as f:
    ...
    if len(actual_data) != 0:
        ...
        combine_all = pd.merge(combine_all, conditional_combined,
                               on=['age', 'gender', 'marital_status', 'emp_status', 'Insurance',
                                   'edu_attain', 'pov_status'], how='left')
        combine_all = pd.merge(combine_all, group_by_cond, on=['age', 'gender', 'marital_status', 'emp_status', 'Insurance',
           'edu_attain', 'pov_status'], how='left')
        ...
        with open(os.path.join(data_path, "kl_div_scores_" + "_copula.csv"), "a") as f:
        ...
        association_entropy_final = association_plot(final_county_population_entropy[req_columns], "Maximum entropy " + str(fips))
