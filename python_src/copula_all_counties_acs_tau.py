...
def fit(u_i, dt, total_fc, theta_0, method):
    ...
    weight_values = []
    f_values = []
    res = minimize(max_entropy, jac = jacobian_m, x0=theta_0, args=(u_i, data, total_fc, method), method='L-BFGS-B', options={'disp': True})
    theta_f = res.x
...
for ind, row in read_top_50.iterrows():
    ...
    while tau >= min(prior_all["p"]):
        ...
        with open(os.path.join(data_path, "tae_scores_copula_tau_acs" + "_.csv"), "a") as f:
        ...
        moved_from_abroad_approach = len(final_county_population_combined[final_county_population_combined["geog_mobility"] == "moved from abroad"])
        moved_from_abroad_actual = j_data['geog_mobility']['moved from abroad'][0]

        field_names_infrequent = ["State", "County", "Infrequent_category", "Data", "Count"]
        with open(os.path.join(data_path, "Infrequent_copula_tau_acs" + "_.csv"), "a") as f:
            dictwriter_object = DictWriter(f, fieldnames=field_names_infrequent)
            dictwriter_object.writerow(
                {"State": state_name, "County": county_name,
                 "Infrequent_category": moved_from_abroad_actual, "Data": "Actual","Count": count})
            dictwriter_object.writerow(
                {"State": state_name, "County": county_name,
                 "Infrequent_category": moved_from_abroad_approach, "Data": "Our approach", "Count": count})
            # Close the file object
            f.close()

        if len(actual_data) != 0:
            group_by_actual = actual_data.groupby(req_columns, as_index=False).size().reset_index(drop=True)
            # print(group_by_actual)
            ...
            with open(os.path.join(data_path, "kl_div_scores_tau_acs" + "_copula.csv"), "a") as f:
